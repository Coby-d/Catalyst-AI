...
const SignUp: React.FC<SignUpProps> = ({ onNavigateToSignIn }) => {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSuccessMessage("Account created! Please check your email for a confirmation link to log in.");
      setEmail('');
      setPassword('');
    }
    setIsLoading(false);
    ...
    <div className="bg-gray-900 text-gray-200 flex flex-col h-screen font-sans items-center justify-center p-4">
      <div className="text-center p-8 bg-gray-800/40 border border-gray-700/50 rounded-2xl shadow-2xl max-w-md w-full">
        <div className="flex items-center justify-center mb-6">
            ...
            <h1 className="text-4xl font-bold text-gray-200 tracking-wide">Create Account</h1>
        </div>
        <p className="text-gray-400 mb-8">Join Catalyst AI to start your journey.</p>
        ...
        <form onSubmit={handleEmailSignUp}>
            ...
            {error && (
             ...
             {successMessage && (
                <div className="mt-4 p-3 bg-green-900/40 border border-green-700/50 text-green-300 rounded-lg text-sm text-left">
                    <p>{successMessage}</p>
                </div>
            )}
